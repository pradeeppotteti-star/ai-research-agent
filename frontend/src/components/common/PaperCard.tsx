  ...
  return (
    <>
      ...
      >
        <div className="flex items-start justify-between gap-4 mb-3">
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap mb-2">
              <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-800 dark:text-emerald-300 border border-emerald-500/20">
                {paper.source}
              </span>
              <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">{paper.publicationDate}</span>
              {paper.openAccess ? (
                <span className="inline-flex items-center gap-1 text-xs text-emerald-700 dark:text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20">
                  <Unlock className="w-3 h-3" /> Open Access
                </span>
              ) : (
                <span className="inline-flex items-center gap-1 text-xs text-amber-700 dark:text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-full border border-amber-500/20">
                  <Lock className="w-3 h-3" /> Metadata Only
                </span>
              )}
              {paper.relevanceScore && (
                <span className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-800 dark:text-emerald-300 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
                  <Award className="w-3.5 h-3.5" /> Score: {Math.round(paper.relevanceScore * 100)}%
                </span>
              )}
            </div>

            <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors line-clamp-2">
              {paper.title}
            </h3>
            ...
            <button
              ...
              className={`p-2 rounded-lg border transition-all ${
                saved
                  ...
                  : 'bg-stone-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-800 hover:text-slate-800 dark:hover:text-slate-200'
              }`}
        ...
        <p className="text-xs text-slate-600 dark:text-slate-300 line-clamp-3 leading-relaxed mb-4">{paper.abstract}</p>

        <div className="flex items-center justify-between pt-3 border-t border-emerald-900/10 dark:border-slate-800/80 text-xs">
          <span className="font-bold text-emerald-700 dark:text-emerald-400 hover:text-emerald-800 flex items-center gap-1">
            View Details & AI Summary →
          </span>

          <button
            onClick={handlePdfClick}
            className="text-emerald-700 dark:text-emerald-400 hover:text-emerald-800 font-bold flex items-center gap-1.5 px-3 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/20"
          >
