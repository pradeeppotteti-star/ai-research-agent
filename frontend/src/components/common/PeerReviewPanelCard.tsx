...
export const PeerReviewPanelCard: React.FC<PeerReviewPanelCardProps> = ({ panel }) => {
  const getDecisionBadge = (decision: PeerReviewPanel['overallDecision']) => {
    switch (decision) {
      ...
      case 'Accept':
        return 'bg-brand-500/10 text-brand-800 dark:text-brand-400 border-brand-500/30';
      case 'Weak Accept':
  ...
  return (
    <div className="glass-panel p-8 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800/80 pb-6">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-700 dark:text-indigo-400 text-xs font-bold mb-2">
            <Award className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" />
            <span>NeurIPS / ICML Benchmark Standard</span>
          </div>
          <h3 className="text-2xl font-black text-slate-900 dark:text-white">Simulated AI Peer-Reviewer Panel</h3>
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            <span className="text-xs text-slate-600 dark:text-slate-400 font-semibold block">Panel Recommendation</span>
          ...
          <div className="w-14 h-14 rounded-2xl bg-indigo-500/10 dark:bg-indigo-600/20 border border-indigo-500/30 dark:border-indigo-500/40 text-indigo-700 dark:text-indigo-300 font-mono font-black text-xl flex items-center justify-center">
        ...
        {panel.reviewers.map((rev, idx) => (
          <div key={idx} className="glass-card p-5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-slate-900 dark:text-slate-200">{rev.reviewerRole}</span>
              <span className="px-2 py-0.5 rounded text-xs font-mono font-bold bg-slate-100 text-brand-700 dark:bg-slate-950 dark:text-brand-400 border border-slate-200 dark:border-slate-800">
                Score: {rev.score}/10
              </span>
            </div>

            <p className="text-xs text-slate-800 dark:text-slate-300 leading-relaxed font-sans">{rev.summary}</p>
            ...
            <div className="space-y-2 pt-2 border-t border-slate-200 dark:border-slate-800/60 text-xs">
              <div>
                <span className="font-bold text-emerald-700 dark:text-emerald-400 flex items-center gap-1 mb-1">
                  <ThumbsUp className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> Key Strengths:
                </span>
                <ul className="list-disc list-inside space-y-1 text-slate-800 dark:text-slate-300 pl-1 font-sans">
              ...
              <div>
                <span className="font-bold text-amber-700 dark:text-amber-400 flex items-center gap-1 mb-1">
                  <AlertCircle className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400" /> Peer Suggestions:
                </span>
                <ul className="list-disc list-inside space-y-1 text-slate-700 dark:text-slate-400 pl-1 font-sans">
