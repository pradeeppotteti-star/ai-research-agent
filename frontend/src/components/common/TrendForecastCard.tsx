    ...
    <div className="glass-panel p-8 rounded-3xl border border-slate-800 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/80 pb-6">
        <div>
          ...
          <h3 className="text-2xl font-black text-white">Research Trend & Impact Forecasting</h3>
        </div>

        <div className="flex items-center gap-3">
          <span className="text-xs font-semibold text-slate-400">Projected 3-Yr Topic Momentum:</span>
          <span className="px-3.5 py-1.5 rounded-xl font-mono font-black text-sm bg-emerald-500/10 text-emerald-400 border border-emerald-500/30">
        ...
        {forecast.emergingTopics.map((topic, idx) => (
          <div key={idx} className="glass-card p-5 rounded-2xl border border-slate-800 space-y-3">
            <div className="flex items-center justify-between">
              <span className="font-bold text-slate-100 text-sm">{topic.keyword}</span>
              {topic.momentum === 'high_growth' || topic.momentum === 'emerging' ? (
                <span className="inline-flex items-center gap-1 text-xs font-bold font-mono text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
                  <ArrowUpRight className="w-3.5 h-3.5" /> +{topic.growthPct}% YoY
                </span>
              ) : (
                <span className="inline-flex items-center gap-1 text-xs font-bold font-mono text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded border border-rose-500/20">
                ...
                {topic.historicalCounts.map((h, i) => {
                  ...
                  return (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1">
                      ...
                      />
                      <span className="text-[9px] font-mono text-slate-500">{h.year}</span>
                    </div>
                  );
                })}
              </div>
      ...
      <div className="bg-brand-950/20 p-5 rounded-2xl border border-brand-500/20 flex items-start gap-3">
        <Sparkles className="w-5 h-5 text-brand-400 shrink-0 mt-0.5" />
        <div>
          <span className="text-xs font-bold text-brand-300 block mb-0.5">Strategic Research Recommendation</span>
          <p className="text-xs text-slate-300 leading-relaxed font-sans">{forecast.recommendation}</p>
        </div>
      </div>
    </div>
  );
