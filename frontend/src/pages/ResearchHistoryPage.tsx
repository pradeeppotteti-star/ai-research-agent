...
export const ResearchHistoryPage: React.FC = () => {
  ...
  return (
    <div className="space-y-8 py-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-brand-400 uppercase tracking-wider mb-1">
            <History className="w-4 h-4" /> Session Repository
          </div>
          <h1 className="text-3xl font-black text-white">Research Session History</h1>
      ...
      {loading ? (
        <div className="py-12 text-center text-slate-400">Loading session history...</div>
      ) : filteredSessions.length === 0 ? (
        <div className="glass-panel p-12 rounded-2xl border border-slate-800 text-center space-y-4">
          <p className="text-slate-400">No matching research sessions found.</p>
          <Link
            to="/research/new"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {filteredSessions.map((s) => (
            ...
            >
              ...
              <h3 className="text-lg font-bold text-slate-100 group-hover:text-brand-300 transition-colors line-clamp-2 mb-3">
                {s.query}
              </h3>
