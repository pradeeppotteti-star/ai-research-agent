...
export const SavedPapersPage: React.FC = () => {
  ...
  return (
    <div className="space-y-8 py-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-bold text-indigo-700 dark:text-indigo-400 uppercase tracking-wider mb-1">
            <Bookmark className="w-4 h-4 text-indigo-600 dark:text-indigo-400" /> Personal Library
          </div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white">Saved Papers Library</h1>
      ...
      {loading ? (
        <div className="py-12 text-center text-slate-600 dark:text-slate-400 font-mono">Loading saved papers...</div>
      ) : filteredPapers.length === 0 ? (
        <div className="glass-panel p-12 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-center text-slate-600 dark:text-slate-400 font-medium">
